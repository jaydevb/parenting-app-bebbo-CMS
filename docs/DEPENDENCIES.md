...
Applied by `cweagans/composer-patches`. `composer-exit-on-patch-failure: true` — a failed patch aborts install. `patchLevel` for `drupal/core` is `-p2`. **All 19 local patch files were confirmed present on disk**; 3 patches are fetched from drupal.org URLs.
