...
| `drupal/warmer` | CDN warmer aborts cold pages at the 30s http_client default timeout — package still declared and patched, but the module is not enabled on any site; warming is done by the custom `bebbo_custom_general` warmer | local |
